...
def write_csv(
    path: Path, headers: list[str], rows: list[list[object]], delimiter: str = ","
) -> None:
...
class ArvineTests(unittest.TestCase):
    ...
    def test_new_trips_snapshot_default_accounting_profile(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                trip_accounting_profile(root, first)["counter_account"], "Corporate Card Payable"
            )
            ...
            self.assertEqual(
                trip_accounting_profile(root, second)["counter_account"], "New Default Payable"
            )
    ...
    def test_arvine_receipt_retains_meal_lines_and_extracts_canadian_tax_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(all(not item.alcohol_reason for item in expense.line_items))
    ...
    def test_four_provider_formats_normalize_to_acceptance_counts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_csv(
                ...
                [
                    ["2026-07-01", "2026-07-02", f"MERCHANT {i}", "A Person", "10001234", i + 1]
                    for i in range(24)
                ],
            )
            ...
            with bmo.open("w", newline="", encoding="utf-8") as handle:
                ...
                for i in range(29):
                    writer.writerow(
                        ["55556666", "DEBIT", "2026-07-03", -(i + 1), f"BMO PURCHASE {i}"]
                    )
            ...
            write_csv(
                ...
                [
                    ["2026-07-04", "99990000", f"BNC PURCHASE {i}", "Travel", i + 1, ""]
                    for i in range(17)
                ],
                delimiter=";",
            )
    ...
    def test_bnc_debit_export_uses_bnc_adapter_and_audits_non_purchases(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            by_type = {
                transaction.transaction_type: transaction for transaction in result.transactions
            }
    ...
    def test_standard_csv_normalizes_exact_cad_refunds_and_audit_rows(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.errors, [])
            by_description = {
                transaction.description: transaction for transaction in result.transactions
            }
    ...
    def test_normalization_signs_audit_rows_and_wise_cad_completeness(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            split = [
                ...
                if item.transaction_group_id == "CARD_TRANSACTION-1"
            ]
            ...
            refund = next(
                item
                for item in result.transactions
                if item.transaction_group_id == "CARD_TRANSACTION-2"
            )
            self.assertEqual(refund.transaction_type, "refund")
            ...
            cancelled = next(
                item
                for item in result.transactions
                if item.transaction_group_id == "CARD_TRANSACTION-3"
            )
    ...
    def test_payments_deposits_cashback_refunds_and_generic_debit_credit_are_auditable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            generic_rows = [
                transaction
                for transaction in result.transactions
                if transaction.provider == "generic"
            ]
            self.assertEqual(
                [transaction.settlement_amount for transaction in generic_rows], [12, -5, -100]
            )
            self.assertEqual(
                [transaction.match_eligible for transaction in generic_rows], [True, True, False]
            )
    ...
    def test_deduplication_only_removes_stable_wise_legs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                all(
                    item.normalization_status == "possible_duplicate"
                    for item in ambiguous.transactions
                )
            )
    ...
    def test_generic_csv_detects_cp1252_semicolon_preamble_and_european_numbers(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trip = ensure_trip(root, "202607_european", mode="arvine")
            statement = trip / "card_statements" / "european.csv"
            statement.write_bytes(
                (
                    "Relevé de carte;Août 2026\n"
                    "Booking Date;Merchant details;Debit amount;Credit amount;Currency;Card No\n"
                    "13.07.2026;Café Montréal;1.234,56;;CAD;1234\n"
                    "14.07.2026;Merchant refund;;25,40;CAD;1234\n"
                    "Booking Date;Merchant details;Debit amount;Credit amount;Currency;Card No\n"
                    ";Total;1.209,16;;;\n"
                ).encode("cp1252")
            )

            report = preflight_statement_files([statement])[0]
            self.assertEqual(report.encoding, "cp1252")
            self.assertEqual(report.delimiter, ";")
            self.assertEqual(report.header_row, 2)
            self.assertEqual(report.rows_normalized, 2)
            self.assertEqual(report.rows_skipped, 2)
            self.assertFalse(report.errors)

            transactions = normalize_statement_files([statement]).transactions
            self.assertEqual(
                [transaction.settlement_amount for transaction in transactions],
                [1234.56, -25.40],
            )
            self.assertEqual(
            ...
            self.assertTrue(
                all(transaction.account_label == "••••1234" for transaction in transactions)
            )

    def test_utf16_tab_csv_infers_purchase_and_settlement_columns(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "multicurrency.csv"
            text = (
                "Posted On\tCard Transaction\tLocal Amount\tLocal Currency\t"
                "Billing Amount\tBilling Currency\n"
                "2026-07-15\tTOKYO HOTEL\t10000\tJPY\t92.50\tCAD\n"
            )
            path.write_bytes(text.encode("utf-16"))

            report = preflight_statement_files([path])[0]
            self.assertEqual(report.encoding, "utf-16")
            self.assertEqual(report.delimiter, "\t")
            self.assertFalse(report.errors)
            transaction = normalize_statement_files([path]).transactions[0]
            self.assertEqual(transaction.purchase_amount, 10000)
            self.assertEqual(transaction.purchase_currency, "JPY")
            self.assertEqual(transaction.settlement_amount, 92.50)
            self.assertEqual(transaction.settlement_currency, "CAD")
            self.assertEqual(transaction.cad_amount, 92.50)

    def test_ambiguous_csv_mapping_is_blocked_then_reused_by_structure(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trip = ensure_trip(root, "202607_custom-csv", mode="arvine")
            folder = trip / "card_statements"
            first = folder / "first.csv"
            first.write_text(
                "Export generated by Example Card\n"
                "When,Merchant Label,Txn Value,ISO Code\n"
                "13/07/2026,TRAIN,-45.20,CAD\n",
                encoding="utf-8",
            )

            blocked = preflight_statement_files([first])[0]
            self.assertTrue(blocked.mapping_required)
            self.assertTrue(blocked.errors)
            self.assertTrue(normalize_statement_files([first]).errors)
            ...
            selected = preflight_statement_files([first])[0]
            self.assertFalse(selected.errors)
            self.assertTrue(selected.profile_reused)
            transaction = normalize_statement_files([first]).transactions[0]
            self.assertEqual(transaction.transaction_type, "purchase")
            self.assertEqual(transaction.settlement_amount, 45.20)

            second = folder / "second.csv"
            second.write_text(
                "When,Merchant Label,Txn Value,ISO Code\n14/07/2026,HOTEL,-120.00,CAD\n",
                encoding="utf-8",
            )
            reused = preflight_statement_files([second])[0]
            self.assertTrue(reused.profile_reused)
            self.assertFalse(reused.errors)
            second_transaction = normalize_statement_files([second]).transactions[0]
            self.assertEqual(second_transaction.description, "HOTEL")
            self.assertEqual(second_transaction.settlement_amount, 120.0)
    ...
    def test_matching_prefers_exact_purchase_currency_and_groups_wise_legs(self):
        ...
        transactions = [
            NormalizedTransaction(
                source_file=Path("wise.csv"),
                source_row=2,
                provider="wise",
                transaction_group_id="GROUP-1",
                funding_leg_id="GROUP-1:a",
    ...
    def test_arvine_workbook_has_review_sheets_formulas_and_editable_matching(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [summary.cell(row, 4).value for row in range(10, 15)],
                [
                    "Travel – Non-meal",
                    "Meals – Deductible (50%)",
    ...
    def test_noninteractive_cli_requires_statement_confirmation_flag(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            trip = ensure_trip(root, "202607_trip", mode="arvine")
            with (
                patch("nlp_expenses.cli.Path.cwd", return_value=root),
                patch("nlp_expenses.cli.sys.stdin.isatty", return_value=False),
                self.assertRaisesRegex(SystemExit, "--statements-complete"),
            ):
